...
export function isKnownAdmissionStatus(value: string): value is AdmissionStatus {
  return admissionStatusValues.includes(value as AdmissionStatus);
}

export function normalizeAdmissionStatus(value: string | undefined | null) {
  const normalized = value?.trim().toUpperCase().replace(/\s+/g, '_') ?? '';
  return isKnownAdmissionStatus(normalized) ? normalized : value?.trim() ?? '';
}

export function getAdmissionApiStatus(error: unknown) {
  return isAxiosError(error) ? error.response?.status : undefined;
}

export function getAdmissionApiMessage(error: unknown, fallback: string) {
...
export function getAdmissionPatientName(patient?: Partial<Admission['patient']> | null) {
...
export function getAdmissionDateValue(admission: Partial<Admission>) {
  return (
...
export function formatAdmissionDate(value: string | undefined, language: string) {
  if (!value) {
    return '';
  }
...
export function getAdmissionRoomLabel(room?: AdmissionRoom | null) {
