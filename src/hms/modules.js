...
import { deepCamelCaseKeys, formatCurrency, formatDate, formatPersonName, getStatusVariant, getValue, normalizeArrayResponse, normalizeRoles, stripEmptyValues, } from './lib/utils';
...
function renderBooleanStatus(value, activeText, inactiveText) {
    const isEnabled = value === true || String(value).toLowerCase() === 'true';
    return _jsx(Badge, { variant: isEnabled ? 'success' : 'secondary', children: isEnabled ? activeText : inactiveText });
}
...
function createReceptionistSchema(mode) {
    return z.object({
        firstName: requiredString(),
        lastName: requiredString(),
        email: emailRule(),
        username: optionalTrimmedString().refine((value) => !value || value.trim().length >= 3, usernameMinText),
        password: mode === 'create'
            ? passwordRule()
            : optionalTrimmedString().refine((value) => !value || value.trim().length >= 6, passwordMinText),
...
function buildReceptionistPayload(values, mode) {
    const payload = {
        firstName: String(values.firstName || '').trim(),
        lastName: String(values.lastName || '').trim(),
        email: String(values.email || '').trim(),
...
function isReceptionistUser(item) {
    return normalizeRoles(getValue(item, 'roles')).includes('RECEPTIONIST');
}
function matchesReceptionistSearch(item, search) {
...
function compareReceptionists(a, b, sortBy, order) {
...
const receptionistService = {
    list: async (params = {}) => {
        ...
        const filtered = users
            .filter((item) => matchesReceptionistSearch(item, search))
            .filter((item) => {
            if (!isActive) {
                return true;
            }
            ...
            .sort((left, right) => compareReceptionists(left, right, sortBy, order));
...
export const moduleConfigs = {
    ...
    receptionists: {
        ...
        description: lt('Manage receptionist accounts, access status, and front desk logins.', 'Verwalten Sie Rezeptionistenkonten, Zugriffsstatus und Frontdesk-Anmeldungen.'),
        endpoint: '/api/auth/users',
