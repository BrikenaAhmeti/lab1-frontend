...
const Button = ({ variant = 'primary', size = 'md', loading = false, leftIcon, rightIcon, children, className, disabled, ...rest }) => {
    return (_jsxs("button", { disabled: disabled || loading, className: clsx('inline-flex items-center justify-center whitespace-nowrap rounded-xl font-semibold transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/45 focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:cursor-not-allowed disabled:opacity-60', size === 'sm' && 'h-9 px-3 text-xs', size === 'md' && 'h-10 px-4 text-sm', size === 'lg' && 'h-12 px-5 text-sm', variant === 'primary' &&
            ...
            'border border-border bg-transparent text-foreground hover:bg-muted/75', variant === 'danger' &&
