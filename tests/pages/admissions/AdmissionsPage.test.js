...
vi.mock('@/domain/admissions/admissions.hooks', () => ({
    useAdmissions: (params) => mockUseAdmissions(params),
...
describe('AdmissionsPage', () => {
    ...
    it('passes the status filter to the admissions query and disables unavailable rooms', () => {
        render(_jsx(MemoryRouter, { initialEntries: ['/app/admissions?status=ACTIVE'], children: _jsx(AdmissionsPage, {}) }));
        ...
        expect(fullRoomOption.disabled).toBe(true);
        expect(maintenanceRoomOption.disabled).toBe(true);
    });
    it('submits a new admission and allows discharging active items', async () => {
        render(_jsx(MemoryRouter, { initialEntries: ['/app/admissions'], children: _jsx(AdmissionsPage, {}) }));
        ...
        fireEvent.change(document.getElementById('admissionDate'), {
